...
../julia --precompiled=no --code-coverage=all -e 'using CoverageBase; CoverageBase.runtests(CoverageBase.testnames())'
# Run coverage without inlining, to test the rest of base
../julia --precompiled=no --inline=no --code-coverage=all -e 'using CoverageBase; CoverageBase.runtests(CoverageBase.testnames())'
cd ..
